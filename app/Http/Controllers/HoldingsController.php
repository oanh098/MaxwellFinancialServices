...
class HoldingsController extends Controller
{
    ...
    public function editUserHoldings($user1){
           ...
           $addHoldings=DB::table('holdings')
            ...
            //dd($holdings);
            //dd($user1);
       // $profiles = DB::select('select * from profiles where id = ?', array(1));
        //$profiles = DB::select( DB::raw("select * from profiles where username = :username"), array('username' => Input::get("username")))
